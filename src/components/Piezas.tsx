  ...
  return (
    <section className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] lg:grid-cols-2 gap-4">
      {listaPiezas.map((pieza, index) => (
        <article
          key={index}
          className={`relative border border-main rounded-xl aspect-video overflow-hidden flex items-end`}
        >
          <div className="relative flex-1 flex z-2 p-4 items-center justify-between">
            ...
            <a
              ...
              className="flex gap-1 item-center p-2 bg-btn hover:bg-btn-hover transition-colors rounded-lg select-none"
            >
              <FaIcon name="download" />
              <span className=" text-icon">Descargar</span>
            </a>
          </div>
          <img
            src={`img/${pieza}.png`}
